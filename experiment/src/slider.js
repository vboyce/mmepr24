...
class SurveySliderPlugin {
  ...
  trial(display_element, trial) {
    ...
    // require responses
    if (trial.require_movement.some(Boolean)) {
      ...
      function check_reponses() {
        ...
        for (var i = 0; i < all_sliders.length; i++) {
          if (
            trial.require_movement[i] &
            !all_sliders[i].classList.contains("clicked")
          ) {
            // if any one slider doesn't have the 'clicked' class and it's supposed to
            //, then we know that they haven't all been clicked
            all_clicked = false;
            break;
          }
        }
