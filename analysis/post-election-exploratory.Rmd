...
if (require("binomialCRIs")) {
  ...
  plot_binomial_hpdi(
    ...
    labs(
      title = "Binomial HDPI check for order effect (in pre-election data)",
      subtitle = paste(
...
cloze |>  filter(response_type!="OTHER") |>
  ...
  ggplot(aes(category, fill = response_type, linetype=batch)) +
  geom_bar(position=position_dodge(), lwd=1, color="black") +
  facet_wrap( ~ paste0(
    sprintf("%02d", item), ":\n",
    gsub("(.{40})", "\\1\n", partial)
  )) +
  scale_fill_response_types +
  labs(
    title = "Type of cloze responses, by item",
    subtitle = "Looking the responses to each item separately (OTHER responses not shown)"
  ) +
...
p_maze2 <- maze |>
  ...
  scale_color_pronouns +
  labs(
    x = "nth_pronoun . batch", y= "mean log residualized RT",
