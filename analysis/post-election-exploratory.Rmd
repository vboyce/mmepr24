...
p_maze_ex <- maze_expectations |>
  ...
  filter(
    probability_Harris >= quantile(distinct(select(., probability_Harris))$probability_Harris, .05),
    probability_Harris <= quantile(distinct(select(., probability_Harris))$probability_Harris, .95),
  ) |>
