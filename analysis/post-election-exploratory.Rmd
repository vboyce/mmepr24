...
for (BATCH in c("pre", "post")) {
  cat(paste0("------------- Batch: ", BATCH, "-election\n"))

  p_expectation <- expectations |>
    filter(batch == BATCH) |>
  ...
  plot_task_expectations_hist <- function(task_expectations, chosen_batch) {
    task_expectations |>
      ...
      select(probability_Trump, workerid, expect) |>
      distinct() |>
  ...
  p_expectation_hist <- expectations |>
    pivot_wider(names_from = candidate, values_from = c(probability, response)) |>
    plot_task_expectations_hist(BATCH)
  ggsave(file.path(outdir_poster, glue::glue("expectation_hist_{BATCH}.pdf")), width = 3.5, height = 2)
  pagebreak()
  print(p_expectation_hist)
  cat("Event expectations histogram\n")

  p_expectation_hist_bytask <- expectations |>
    filter(batch == BATCH) |>
    ...
    labs(x = "Probability Trump")
  pagebreak()
  print(p_expectation_hist_bytask)
  cat("Event expectations histogram by task\n")

  ## CLOZE
  p_cloze <- cloze_categorized |>
  ...
  p_cloze_expect <- cloze_expectations |>
    filter(batch == BATCH) |>
    mutate(
      expect = factor(case_when(
  ...
  if (BATCH == "pre") {
    ggsave(
      file.path(outdir_poster, glue::glue("cloze_expect_{BATCH}.pdf")),
      width = 4.5, height = 3.5
    )
    cat("Cloze by expectations\n")
    pagebreak()
    print(p_cloze_expect)
  }

  p_cloze_expectation_hist <- cloze_expectations |>
    plot_task_expectations_hist(BATCH)
  ggsave(file.path(outdir_poster, glue::glue("cloze_expectation_hist_{BATCH}.pdf")), width = 3.5, height = 2)
  pagebreak()
  print(p_cloze_expectation_hist)
  cat("Cloze task expectations histogram\n")
  ...
  pagebreak()
  print(p_maze2)
  cat("Maze second pronoun\n")

  p_maze_expect <- maze_expectations |>
    filter(batch == BATCH) |>
    mutate(
      pronoun_gender = factor(pronoun_gender, levels = c("she", "he", "they")),
      expect = factor(case_when(
  ...
  if (BATCH == "pre") {
    ggsave(
      file.path(outdir_poster, glue::glue("maze_expect_{BATCH}.pdf")),
      width = 3.5, height = 2.5
    )
    pagebreak()
    print(p_maze_expect)
    cat("Maze by expectation\n")
  }

  p_maze_expectation_hist <- maze_expectations |>
    plot_task_expectations_hist(BATCH)
  ggsave(file.path(outdir_poster, glue::glue("maze_expectation_hist_{BATCH}.pdf")), width = 3.5, height = 2)
  pagebreak()
  print(p_maze_expectation_hist)
  cat("Maze task expectations histogram\n")
  ...
  pagebreak()
  print(p_spr2)
  cat("SPR second pronoun\n")

  p_spr_expect <- spr_expectations |>
    filter(batch == BATCH) |>
    mutate(
      pronoun_gender = factor(pronoun_gender, levels = c("she", "he", "they")),
      expect = factor(case_when(
  ...
  if (BATCH == "pre") {
    ggsave(
      file.path(outdir_poster, glue::glue("spr_expect_{BATCH}.pdf")),
      width = 3.5, height = 2.5
    )
    pagebreak()
    print(p_spr_expect)
    cat("SPR by expectations\n")
  }

  p_spr_expectation_hist <- spr_expectations |>
    plot_task_expectations_hist(BATCH)
  ggsave(file.path(outdir_poster, glue::glue("spr_expectation_hist_{BATCH}.pdf")), width = 3.5, height = 2)
  pagebreak()
  print(p_spr_expectation_hist)
  cat("SPR expectations histogram\n")
}
```
...
Other exploratory analysis below.
