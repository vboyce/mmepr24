...
response_type_colors <- c(
  ...
  "hedged" = "darkgreen", "pronoun neutral" = "#15bb36",
  "ambiguous NP" = "#293629",
  "OTHER" = "gray"
)
...
```{r}
F_count <- nrow(filter(cloze, batch=="pre", response_type=="pronoun female"))
M_count <- nrow(filter(cloze, batch=="pre", response_type=="pronoun male"))
...
p_maze_ex <- maze_expectations |>
  ...
  # filter(
  #   probability_Harris >= quantile(distinct(select(., probability_Harris))$probability_Harris, .15),
  #   probability_Harris <= quantile(distinct(select(., probability_Harris))$probability_Harris, .85),
  # ) |>
...
# maze_expectations |>
  # filter(correct, is_target, is_pron1, batch == "pre") %>%
  # quantile(distinct(select(., probability_Harris))$probability_Harris, .33)
maze_expectations |>
  filter(correct, is_target, is_pron1, batch == "pre") |>
  mutate(prob_bin = case_when(
    probability_Harris > 0.5 ~ "P(Harris) > 50%",
    probability_Harris < 0.5 ~ "P(Harris) < 50%",
    probability_Harris == 0.5 ~ "P(Harris) = 50%"
  )) |>
  ggplot(aes(prob_bin, log(rt), fill = pronoun_gender)) +
  geom_boxplot() +
  scale_fill_pronouns +
  labs(
    title = "Maze RT on pronouns by belief in Harris presidency",
    x = "Belief about next president",
    y = "Maze RT (log scale)",
    fill = "Pronoun gender"
  )
```
...
ggsave(here("writing/HSP2025/expectation.pdf"), width = 5 , height = 2 )
...
cloze_categorized <- cloze |> # filter(response_type!="OTHER") |>
  mutate(category = case_when(
    ...
    .default = "OTHER"
                                      ...
                                      levels=c("Harris", "pronoun female",
                                               "Trump", "pronoun male",
                                               "hedged", "pronoun neutral",
                                               "ambiguous NP"), 
                                      labels=c("Harris", "she", "Trump", "he",
                                               "hedged", "they", "ambig. NP"))
  ) |> 
  group_by(batch, response_type, category) |> summarize(n=n()) |> group_by(batch) |> mutate(n=n/sum(n))

cloze_categorized |> mutate(batch=factor(batch, levels=c("pre", "post"), labels=c("Pre", "Post"))) |> filter(response_type!="OTHER") |> ungroup() |> 
...
ggsave(here("writing/HSP2025/cloze.pdf"), width = 5, height = 3)
...
ggsave(here("writing/HSP2025/maze.pdf"), width= 5, height =2.5)
...
ggsave(here("writing/HSP2025/maze2.pdf"), width = 5, height =2.5)
...
ggsave(here("writing/HSP2025/spr.pdf"), width= 5, height =2.5)
...
ggsave(here("writing/HSP2025/spr2.pdf"), width = 5, height =2.5)
...
response_type_colors <- c(
  "Harris" = "darkblue", "she" = "blue",
  "Trump" = "darkred", "he" = "red",
  "ambig. NP" = "darkgreen", "they" = "#15bb36",
  "hedged" = "#293629", 
  "OTHER" = "gray"
)
scale_fill_response_types <- scale_fill_manual(
...
cloze_categorized <- cloze |> # filter(response_type!="OTHER") |>
  mutate(category = case_when(
    ...
    response_type %in% c("hedged", "pronoun neutral", "ambiguous NP") ~ "neutral",
    .default = "OTHER"
  )) |> mutate(response_type = factor(
    response_type, 
    levels=c(
      "Harris", "pronoun female","Trump", "pronoun male",
      "hedged","ambiguous NP","pronoun neutral"), 
    labels=c(
      "Harris", "she", "Trump", "he",
      "hedged", "ambig. NP", "they"))
  ) |> 
  group_by(batch, response_type, category) |> summarize(n=n()) |> group_by(batch) |> mutate(n=n/sum(n))
...
```{r}
maze_expectations |> filter(batch=="pre") |>
  mutate(
    pronoun_gender=factor(pronoun_gender, levels=c("she", "he", "they")),
    expect = factor(case_when(
      probability_Harris > .51 ~ "Expect female", 
      probability_Harris < .49 ~ "Expect male",.default="Even"))) |> filter(expect != "Even") |>
  filter(is_target) |>
  filter(is_pron1) |> drop_na(expect) |>
  ggplot(aes(pronoun_gender, log(rt), color=factor(pronoun_gender))) +
  stat_summary(position=position_dodge2(width=.5)) +
  scale_color_pronouns +
  labs(
  ...
  theme(legend.position = "none",
        ...
        axis.title.x=element_blank(),
                legend.text=element_text(size="14"),
        strip.background = element_blank())
ggsave(here("writing/HSP2025/maze3.pdf"), width = 5, height = 2.5)
```
...
spr_expectations |> filter(batch=="pre") |>
  mutate(
    pronoun_gender=factor(pronoun_gender, levels=c("she", "he", "they")),
    expect = factor(case_when(
      probability_Harris > .51 ~ "Expect Harris", 
      probability_Harris <= .49 ~ "Expect Trump",.default="Even"))) |> filter(expect != "Even") |>
  filter(is_target) |>
  filter(is_pron1) |> drop_na(expect) |>
  ggplot(aes(pronoun_gender, log(rt), color=factor(pronoun_gender))) +
  stat_summary(position=position_dodge2(width=.5)) +
...
ggsave(here("writing/HSP2025/spr3.pdf"), width = 5, height = 2.5)
```