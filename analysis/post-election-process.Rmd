---
title: "Process post-election data"
output:
  pdf_document:
    toc: true
  html_document:
...
trials_loc <- "data/mme_post_election-trials.csv"
...
workerids_loc <- "data/mme_post_election-workerids.csv"
prolific_demog_loc <- "data/prolific_export_672e395849eebe24bbd6ec5f.csv"
...
ParseJSONColumn <- function(x) {
  str_replace_all(x, c(
    ...
    '([A-Za-z]+)"s' = "\\1's",
    'States" ' = "States' ",
    ' "([A-Za-z]+)" ' = " `\\1` ", # replace quotes to avoid some issues
...
prolific_demog_raw <- read_csv(here(prolific_demog_loc)) |>
  ...
  mutate(Age = if_else(Age == "CONSENT_REVOKED", NA, Age))
...
demographics <- raw |>
  ...
  # Ad hoc fixes: (second not needed for post-election data. fixed.)
...
demographics |> drop_na(age) |>
  ggplot(aes(as.numeric(age),fill = age_bin)) +
...
demographics_and_prolific_uniq |> 
  ...
  labs(
    ...
    subtitle = str_glue("Start: {format(min(demographics_and_prolific_uniq$`Started at`,na.rm=T)|>with_tz(timezone), '%H:%M, %B %d, %Y')}") # nolint: line_length_linter.
  ) + theme_minimal() +
  ...
  mutate(
    response_type = case_when(
      ...
      str_detect(response, "(?i)\\b(harris|kamala|president harris|president kamala|the first black)") ~
        "Harris",
      str_detect(response, "(?i)\\b(((the|next|u.s.|u.s|us|new(.*)) (president|potus))|the candidate)") ~
        "ambiguous NP",
      TRUE ~ "OTHER"
    )
...
if (FALSE) {
  ...
  cloze |>
    select(item, partial, response_type, response, everything()) |>
    mutate(
      partial = gsub(
    ...
    arrange(response_type,partial) |> #write_excel_csv("TEMP.csv")
...
write_csv(
  recall_question |> mutate(batch = "post"),
  here("data/processed/post_recall_question.csv")
)
write_csv(
  expectations |> mutate(batch = "post"),
  here("data/processed/post_expectations.csv")
)
write_csv(
  cloze |> mutate(batch = "post"),
  here("data/processed/post_cloze.csv")
)
write_csv(
  spr |> mutate(batch = "post"),
  here("data/processed/post_spr.csv")
)
write_csv(
  spr_all |> mutate(batch = "post"),
  here("data/processed/post_spr_all.csv")
)
write_csv(
  maze |> mutate(batch = "post"),
  here("data/processed/post_maze.csv")
)
write_csv(
  participants |> mutate(batch = "post"),
  here("data/processed/post_participants.csv")
)
```
