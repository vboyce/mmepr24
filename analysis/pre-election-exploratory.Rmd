...
```{r}
expectations |> mutate(condition = case_when(
    condition|>str_starts("event-")~"event-[cloze/maze/spr]",
    .default=condition
  )) |> 
  ggplot(aes(probability, candidate, color = candidate)) +
  labs(
...
```{r}
cloze |> left_join(participants) |> # filter(response_type!="OTHER") |>
  mutate(category = case_when(
    response_type %in% c("Harris", "pronoun female") ~ "female",
    response_type %in% c("Trump", "pronoun male") ~ "male",
    response_type %in% c("hedged", "pronoun neutral", "ambiguous NP") ~ "neutral",
  ...
  geom_bar() + 
  geom_text(stat='count', aes(label=..count..), color="white", position=position_stack(vjust=0.5)) +
  facet_wrap(gender~condition, scales="free") +
  scale_fill_response_types +
...
participants |>
  ...
  ggplot(aes(pronoun_gender, log(rt), color = factor(pronoun_gender))) +
  stat_summary() + facet_grid(condition~gender) +
  scale_color_pronouns +
...
participants |>
  ...
  ggplot(aes(pronoun_gender, log(rt), color = factor(pronoun_gender))) +
  stat_summary() + facet_grid(condition~political_aff) +
  scale_color_pronouns +
...
participants |>
  ...
  ggplot(aes(pronoun_gender, log(rt), color = factor(pronoun_gender))) +
  stat_summary() + facet_grid(condition~prefer) +
  scale_color_pronouns +
...
participants |>
  ...
  scale_color_pronouns + facet_grid(condition~.) +
  labs(title="Maze RT on pronouns, plotted against probability of female president",
    x = "belief that president will be Harris", y = "Maze RT (log scale)")
```
