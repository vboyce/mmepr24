...
if (length(workerids_many_from) > 1) {
  warning("Some proliferate workerids are associated with multiple runs. Will exclude.")
  workerids |>
...
demographics_and_prolific_uniq |> drop_na(`Started at`) |>
  mutate(
    across(
  ...
  ggplot(aes(y=workerid)) +
  geom_segment(
    aes(x = `Started at`, xend = `Completed at`, yend = workerid, color=included), alpha = 0.5
  ) +
  labs(
  ...
  ) + theme_minimal() + facet_grid(included~.,scales="free",space="free") +
  theme(
...
write_csv(recall_question, here("data/processed/pre_recall_question.csv"))
write_csv(expectations, here("data/processed/pre_expectations.csv"))
write_csv(cloze, here("data/processed/pre_cloze.csv"))
write_csv(spr, here("data/processed/pre_spr.csv"))
write_csv(spr_all, here("data/processed/pre_spr_all.csv"))
write_csv(maze, here("data/processed/pre_maze.csv"))
write_csv(participants, here("data/processed/pre_participants.csv"))
```
