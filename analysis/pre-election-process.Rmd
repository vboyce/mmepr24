...
sprmaze_stim_s12 <- mutate(filter(sprmaze_stim |> select(-distractor), item != "start"), item = as.numeric(item))
spr_all <- spr_all |>
...
if (length(workerids_many_from) > 1) {
  ...
  workerids |>
    ...
    summarize(paste(prolific_id, collapse = ", "), .by = workerid)
}
...
cloze <- exclude_participants(cloze_all, use_comp_q = F) |>
  ...
  mutate(response_type = case_when(
    workerid %in% c(1676, 2291) & item == 12 ~ "ambiguous NP", # used uncaught animate noun phrase to refer to president ambiguously
    TRUE ~ response_type
  )) |>
...
write_csv(demographics_and_prolific_uniq, here("data/processed/participants.csv"))
```
